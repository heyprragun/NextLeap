...
import { ArrowRight, Award, Bot, BriefcaseBusiness, Check, ChevronRight, CircleDollarSign, Compass, Dumbbell, ExternalLink, HeartHandshake, Menu, Send, ShieldCheck, Sparkles, Target, TrendingUp, Trophy, Wallet, X } from "lucide-react";
import "./styles.css";
...
function App() {
  ...
  return <div className="app">
    ...
    <Mardarshak athlete={dashboard?.athlete} dashboard={dashboard} language={language} />
...
function Mardarshak({ athlete, dashboard, language }) {
  const [open, setOpen] = useState(false); const [input, setInput] = useState(""); const [loading, setLoading] = useState(false);
  const displayName = athlete?.name?.split(" ")[0];
  const [messages, setMessages] = useState([{ role: "assistant", content: language === "hi" ? `नमस्ते${displayName ? ` ${displayName}` : ""}! मैं मार्गदर्शक हूं। अपना खेल, करियर या पैसों से जुड़ा सवाल पूछें।` : `Hi${displayName ? ` ${displayName}` : ""}! I’m Mardarshak. Ask me about your sport, career, or financial readiness.` }]);
  const sendMessage = async (event) => { event.preventDefault(); const message = input.trim(); if (!message || loading) return; const next = [...messages, { role: "user", content: message }]; setMessages(next); setInput(""); setLoading(true); try { const result = await api("/api/chat", { method: "POST", body: JSON.stringify({ athlete: athlete || { name: "Athlete", sport: "not provided", level: "not provided", years: 0, skills: [] }, dashboard, language, message, history: next.slice(-8) }) }); setMessages((current) => [...current, { role: "assistant", content: result.reply }]); } catch (error) { setMessages((current) => [...current, { role: "assistant", content: error.message }]); } finally { setLoading(false); } };
  return <aside className={open ? "mardarshak open" : "mardarshak"}><button className="mardarshakFab" onClick={() => setOpen(!open)} aria-label="Open Mardarshak"><Bot size={19} /><span>Mardarshak</span></button>{open && <div className="mardarshakPanel"><div className="mardarshakHeader"><div><b>मार्गदर्शक · Mardarshak</b><small>Your profile-aware coach</small></div><button onClick={() => setOpen(false)} aria-label="Close chatbot"><X size={16} /></button></div><div className="mardarshakMessages">{messages.map((item, index) => <div className={item.role === "user" ? "chatBubble user" : "chatBubble"} key={`${item.role}-${index}`}><ChatContent content={item.content} /></div>)}{loading && <div className="chatBubble typing"><span className="thinkingIcon"><span /><span /><span /></span><span>Mardarshak is thinking</span></div>}</div><form className="mardarshakForm" onSubmit={sendMessage}><input value={input} onChange={(event) => setInput(event.target.value)} placeholder={language === "hi" ? "अपना सवाल लिखें..." : "Ask about your next step..."} aria-label="Ask Mardarshak" /><button className="primary" disabled={loading || !input.trim()} aria-label="Send question"><Send size={15} /></button></form><small className="chatDisclaimer">Educational guidance, not financial advice.</small></div>}</aside>;
}

function ChatContent({ content }) {
  const inline = (text, keyPrefix) => text.split(/(\*\*[^*]+\*\*|`[^`]+`)/g).filter(Boolean).map((part, index) => part.startsWith("**") ? <strong key={`${keyPrefix}-b-${index}`}>{part.slice(2, -2)}</strong> : part.startsWith("`") ? <code key={`${keyPrefix}-c-${index}`}>{part.slice(1, -1)}</code> : part);
  return <div className="chatFormatted">{String(content).split("\n").map((line, index) => { const trimmed = line.trim(); if (!trimmed) return <div className="chatSpacer" key={`space-${index}`} />; const bullet = trimmed.match(/^[-*•]\s+(.*)$/); const numbered = trimmed.match(/^\d+[.)]\s+(.*)$/); if (bullet) return <div className="chatListItem" key={`line-${index}`}><span>•</span><div>{inline(bullet[1], `line-${index}`)}</div></div>; if (numbered) return <div className="chatListItem" key={`line-${index}`}><span className="chatNumber">{trimmed.match(/^\d+/)[0]}.</span><div>{inline(numbered[1], `line-${index}`)}</div></div>; return <p key={`line-${index}`}>{inline(trimmed, `line-${index}`)}</p>; })}</div>;
}
...
function Dashboard({ data, onRefresh, onToast }) {
  ...
  useEffect(() => { let frame; const start = animatedReadiness; const started = performance.now(); const animate = (now) => { const progress = Math.min(1, (now - started) / 650); setAnimatedReadiness(Math.round(start + (readiness - start) * progress)); if (progress < 1) frame = requestAnimationFrame(animate); }; frame = requestAnimationFrame(animate); return () => cancelAnimationFrame(frame); }, [readiness]);
  return <main className="dashboardPage"><div className="dashTop"><div><div className="eyebrow">ATHLETE DASHBOARD</div><h1>{t("dashboard.welcome")}, {data.athlete.name.split(" ")[0]}.</h1><p>{t("dashboard.subtitle")} {data.stats.completedTasks ? `${data.stats.completedTasks}/6 roadmap tasks complete.` : "Start with one small action today."}</p></div><div className="dashStats"><span>{data.stats.completedCourses} learning goals</span><span>{data.stats.savedOpportunities} saved opportunities</span></div></div>
    <section className="readinessGrid"><div className="readinessCard mainReadiness"><div><span className="muted">OVERALL READINESS</span><strong>{animatedReadiness}%</strong><small>{data.stats.learningStreak ? `Learning streak: ${data.stats.learningStreak} days` : "Based on your current profile"}</small><span className="readinessHint" title="Calculated from profile completeness, mapped skills, financial modules and matched opportunities.">ⓘ {data.readiness.profileCompleteness}% profile complete • {data.readiness.mappedSkills} skills mapped • {data.readiness.completedModules} money modules</span></div><div className="ring" style={{ "--p": `${animatedReadiness}%` }}><span>{animatedReadiness}</span></div></div><Score title={t("dashboard.career")} value={data.readiness.career} icon={<Compass />} /><Score title={t("dashboard.skills")} value={data.readiness.skills} icon={<Target />} /><Score title={t("dashboard.opportunity")} value={data.readiness.opportunity} icon={<BriefcaseBusiness />} /></section>
...
function Roadmap({ data, athleteId, onUpdate, t }) {
  ...
  const phases = ["Days 01–30", "Days 31–60", "Days 61–90"].map((phase) => ({ phase, tasks: data.roadmap.filter((task) => task.phase === phase) }));
  const downloadPdf = () => {
    const pdf = new jsPDF({ unit: "mm", format: "a4" });
    const margin = 18; const pageWidth = 210; let y = 20;
    const addWrapped = (text, size = 10, color = [55, 65, 57], gap = 6) => { pdf.setFontSize(size); pdf.setTextColor(...color); const lines = pdf.splitTextToSize(String(text), pageWidth - margin * 2); pdf.text(lines, margin, y); y += lines.length * (size * 0.45) + gap; };
    pdf.setFillColor(216, 255, 67); pdf.rect(0, 0, pageWidth, 9, "F");
    addWrapped("NEXTLAP", 20, [16, 19, 14], 3); addWrapped("Your personalized 90-day transition plan", 11, [100, 110, 102], 10);
    addWrapped(`Athlete: ${data.athlete.name}`, 11, [16, 19, 14], 4); addWrapped(`Sport: ${data.athlete.sport}  |  Level: ${data.athlete.level}  |  Years: ${data.athlete.years}`, 9, [100, 110, 102], 4);
    const topCareer = data.careerMatches?.[0]?.title || "Career exploration"; addWrapped(`Recommended direction: ${topCareer}`, 10, [16, 19, 14], 4); addWrapped(`Readiness today: ${Math.round((data.readiness.career + data.readiness.financial + data.readiness.skills + data.readiness.opportunity) / 4)}%`, 10, [70, 105, 35], 12);
    phases.forEach((group, index) => { if (y > 255) { pdf.addPage(); y = 20; } pdf.setFillColor(32, 43, 28); pdf.roundedRect(margin, y - 5, pageWidth - margin * 2, 10, 2, 2, "F"); pdf.setTextColor(216, 255, 67); pdf.setFontSize(11); pdf.text(`PHASE 0${index + 1}  ·  ${group.phase}`, margin + 4, y + 2); y += 14; group.tasks.forEach((task) => { if (y > 270) { pdf.addPage(); y = 20; } addWrapped(`${task.done ? "[DONE]" : "[ ]"} ${task.text}`, 10, task.done ? [70, 105, 35] : [55, 65, 57], 5); }); y += 5; });
    addWrapped("NextLap helps athletes turn sporting experience into career and financial resilience.", 8, [110, 120, 112], 4); pdf.save(`nextlap-90-day-plan-${data.athlete.name.replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "").toLowerCase() || "athlete"}.pdf`);
  };
  return <div className="panel roadmap"><div className="panelHead"><div><div className="eyebrow">06 • {t("dashboard.roadmap")}</div><h2>{t("dashboard.roadmapTitle")}</h2><p className="passportIntro">A practical plan shaped around your strongest career match.</p></div><div className="roadmapTools"><TrendingUp /><button className="secondary printButton" onClick={downloadPdf}>Download as PDF</button></div></div><div className="timeline">{phases.map((group, index) => <section className="timelinePhase" key={group.phase}><div className="timelineMarker">0{index + 1}</div><div className="timelineBody"><small>{group.phase}</small><div className="timelineTasks">{group.tasks.map((task) => <label className={task.done ? "road done" : "road"} key={task.id}><input type="checkbox" checked={task.done} onChange={() => toggle(task)} /><div className="roadNo">{task.done ? <Check size={22} /> : ""}</div><h3>{task.text}</h3><span>{task.done ? t("dashboard.completed") : "Next action"}</span></label>)}</div></div></section>)}</div></div>;
}
